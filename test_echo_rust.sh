...
echo ""
echo "=== Step 2: Build Rust server and client ==="
cargo build --release -p rust_echo_server -p rust_echo_client

echo ""
echo "=== Step 3: Start echo server ==="
./target/release/rust_echo_server --port "$PORT" &
SERVER_PID=$!
sleep 2
...
TEST_MESSAGE="Hello from Rust integration test"
OUTPUT=$(./target/release/rust_echo_client --port "$PORT" "$TEST_MESSAGE")
echo "Client output:"
echo "$OUTPUT"
